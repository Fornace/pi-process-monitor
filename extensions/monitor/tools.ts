...
const monitorParams = Type.Object({
  command: Type.Optional(Type.String({ description: "Shell command. Spawn once without intervalSeconds; poll only when it is a read-only observer." })),
  intervalSeconds: Type.Optional(Type.Number({ minimum: 2, description: "Poll cadence; enables poll mode." })),
  logFile: Type.Optional(Type.String({ description: "File to tail for appended matching lines." })),
...
export function registerTools(pi: ExtensionAPI, runtime: MonitorRuntime): void {
  pi.registerTool({
    ...
    description: "Start or exactly reuse a non-blocking spawn, structured observer, remote poll, or file tail. Local shell polls are quarantined unless explicitly acknowledged as observers.",
    ...
    async execute(_id, params: MonitorParams, _signal, _update, ctx) {
      if ([params.command, params.logFile, params.probe].filter(Boolean).length !== 1) throw new Error("provide exactly one source: command, logFile, or probe");
      const config = {
